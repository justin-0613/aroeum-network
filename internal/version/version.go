...
// conventions in the Aroeum p2p network.
...
//
// If the package that is currently executing is a prefixed by our go-aroeum
...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "garo".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("garo %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("garo %s", params.VersionWithMeta)
	}
