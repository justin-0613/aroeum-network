// Copyright 2015 The go-aroeum Authors
...
// AroeumAPI provides an API to access Aroeum related information.
type AroeumAPI struct {
	b Backend
}

// NewAroeumAPI creates a new Aroeum protocol API.
func NewAroeumAPI(b Backend) *AroeumAPI {
	return &AroeumAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *AroeumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *AroeumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *AroeumAPI) FeeHistory(ctx context.Context, blockCount math.HexOrDecimal64, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *AroeumAPI) Syncing() (interface{}, error) {
...
// AroeumAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type AroeumAccountAPI struct {
	am *accounts.Manager
}

// NewAroeumAccountAPI creates a new AroeumAccountAPI.
func NewAroeumAccountAPI(am *accounts.Manager) *AroeumAccountAPI {
	return &AroeumAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *AroeumAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Aroeum ECDSA signature for:
// keccak256("\x19Aroeum Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x19Aroeum Signed Message:\n"${message length}${message})
...
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Aroeum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and garo.
...
// BlockChainAPI provides an API to access Aroeum blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Aroeum blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Aroeum chain config.
//
...
// DebugAPI is the collection of Aroeum APIs exposed over the debugging
// namespace.
...
// Version returns the current aroeum protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Aro)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f aro) exceeds the configured cap (%.2f aro)", feeFloat, cap)
	}
	return nil
}
