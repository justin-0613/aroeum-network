...
// Package garoclient provides an RPC client for garo-specific APIs.
package garoclient
...
// Client is a wrapper around rpc.Client that implements garo-specific functionality.
//
// If you want to use the standardized Aroeum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg aroeum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg aroeum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a garo node.
...
// MemStats retrieves the current memory stats from a garo node.
...
// GetNodeInfo retrieves the node info of a garo node.
