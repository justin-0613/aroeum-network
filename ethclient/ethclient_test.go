...
// Verify that Client implements the aroeum interfaces.
var (
	_ = aroeum.ChainReader(&Client{})
	_ = aroeum.TransactionReader(&Client{})
	_ = aroeum.ChainStateReader(&Client{})
	_ = aroeum.ChainSyncReader(&Client{})
	_ = aroeum.ContractCaller(&Client{})
	_ = aroeum.GasEstimator(&Client{})
	_ = aroeum.GasPricer(&Client{})
	_ = aroeum.LogFilterer(&Client{})
	_ = aroeum.PendingStateReader(&Client{})
	// _ = aroeum.PendingStateEventer(&Client{})
	_ = aroeum.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Aroeum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new aroeum service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == aroeum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != aroeum.NotFound {
		t.Fatal("error should be aroeum.NotFound")
	}
}
