...
// Package utils contains internal helper functions for go-aroeum commands.
package utils
...
import (
	...
	ethcatalyst "github.com/aroeum-network/go-aroeum/eth/catalyst"
	...
	"github.com/aroeum-network/go-aroeum/ethdb/remotedb"
	"github.com/aroeum-network/go-aroeum/ethstats"
	"github.com/aroeum-network/go-aroeum/graphql"
	...
	lescatalyst "github.com/aroeum-network/go-aroeum/les/catalyst"
	"github.com/aroeum-network/go-aroeum/log"
	"github.com/aroeum-network/go-aroeum/metrics"
	"github.com/aroeum-network/go-aroeum/metrics/exp"
	"github.com/aroeum-network/go-aroeum/metrics/influxdb"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in aro) that can be sent via the RPC APIs (0 = no cap)",
	...
	HttpHeaderFlag = &cli.StringSliceFlag{
		Name:     "header",
		Aliases:  []string{"H"},
		Usage:    "Pass custom headers to the RPC server when using --" + RemoteDBFlag.Name + " or the garo attach console. This flag can be given multiple times.",
		Category: flags.APICategory,
	}
...
// for Garo and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `garo account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Aroeum client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Aroeum) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Aroeum service: %v", err)
		}
...
// RegisterEthStatsService configures the Aroeum Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Aroeum Stats service: %v", err)
	}
}
...
func RegisterFullSyncTester(stack *node.Node, eth *eth.Aroeum, path string) {
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "garo.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "garo.", tagsMap)
		}
...
func DialRPCWithHeaders(endpoint string, headers []string) (*rpc.Client, error) {
	...
	if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with garo < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
