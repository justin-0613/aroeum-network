...
func loadConfig(file string, cfg *garoConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "garo.ipc"
	return cfg
}

// makeConfigNode loads garo configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, garoConfig) {
	// Load defaults.
	cfg := garoConfig{
...
// makeFullNode loads garo configuration and creates the Aroeum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	// Add the Aroeum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *garoConfig) {
