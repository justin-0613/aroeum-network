...
`
	...
	{
		garo := runGaro(t, "account", "list", "--datadir", datadir)
		garo.Expect(want)
		garo.ExpectExit()
	}
	{
		garo := runGaro(t, "--datadir", datadir, "account", "list")
		garo.Expect(want)
		garo.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	garo := runGaro(t, "account", "new", "--lightkdf")
	defer garo.ExpectExit()
	garo.Expect(`
...
func TestAccountHelp(t *testing.T) {
	garo := runGaro(t, "account", "-h")
	garo.WaitExit()
	if have, want := garo.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	garo = runGaro(t, "account", "import", "-h")
	garo.WaitExit()
	if have, want := garo.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	garo := runGaro(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer garo.ExpectExit()
	garo.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	garo := runGaro(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer garo.ExpectExit()
	garo.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(garo.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	garo := runMinimalGaro(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(garo.StderrText(), m) {
...
// https://github.com/aroeum-network/go-aroeum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	garo.SetTemplateFunc("keypath", func(file string) string {
