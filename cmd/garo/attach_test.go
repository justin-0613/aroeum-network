...
// TestAttachWithHeaders tests that 'garo attach' with custom headers works, i.e
...
// TestAttachWithHeaders tests that 'garo db --remotedb' with custom headers works, i.e
// that custom headers are forwarded to the target.
...
func testReceiveHeaders(t *testing.T, ln net.Listener, garoArgs ...string) {
	var ok uint32
	server := &http.Server{
		Addr: "localhost:0",
