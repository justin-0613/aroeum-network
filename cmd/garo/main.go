...
// garo is the official command-line client for Aroeum.
package main
...
import (
	...
	_ "github.com/aroeum-network/go-aroeum/eth/tracers/js"
	_ "github.com/aroeum-network/go-aroeum/eth/tracers/native"

	"github.com/urfave/cli/v2"
)

const (
	clientIdentifier = "garo" // Client identifier to advertise over the network
)
...
var app = flags.NewApp("the go-aroeum command line interface")

func init() {
	// Initialize the CLI app and start Garo
	app.Action = garo
	app.Copyright = "Copyright 2013-2023 The go-aroeum Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Garo on Rinkeby testnet...")

	case ctx.IsSet(utils.GoerliFlag.Name):
		log.Info("Starting Garo on Görli testnet...")

	case ctx.IsSet(utils.SepoliaFlag.Name):
		log.Info("Starting Garo on Sepolia testnet...")

	case ctx.IsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Garo in ephemeral dev mode...")
		log.Warn(`You are running Garo in --dev mode. Please note the following:
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Garo on Aroeum mainnet...")
	}
...
// garo is the main entry point into the system if no special subcommand is run.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func garo(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local garo node.
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) || ctx.Bool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Aroeum node is running
		if ctx.String(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("Aroeum service not running")
		}
