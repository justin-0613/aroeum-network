...
Use `devp2p enrdump <base64>` to verify and display an Aroeum Node Record.
...
1. initialize the garo node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run garo with the following flags:
```
garo --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the garo node:
 ```
...
To run the eth66 protocol test suite, initialize a garo node as described above and run the following command,
replacing `<enode>` with the enode of the garo node:
