...
// Package les implements the Light Aroeum Subprotocol.
package les
...
import (
	...
	"github.com/aroeum-network/go-aroeum/les/downloader"
	"github.com/aroeum-network/go-aroeum/les/vflux"
	vfc "github.com/aroeum-network/go-aroeum/les/vflux/client"
...
func New(stack *node.Node, config *ethconfig.Config) (*LightAroeum, error) {
...
func (s *LightAroeum) VfluxRequest(n *enode.Node, reqs vflux.Requests) vflux.Replies {
	if !s.udpEnabled {
		return nil
	}
...
func (s *LightAroeum) vfxVersion(n *enode.Node) uint {
	if n.Seq() == 0 {
		var err error
		if !s.udpEnabled {
			return 0
...
func (s *LightAroeum) prenegQuery(n *enode.Node) int {
	if s.vfxVersion(n) < 1 {
...
// APIs returns the collection of RPC services the aroeum package offers.
// NOTE, some of these services probably need to be moved to somewhere else.
func (s *LightAroeum) APIs() []rpc.API {
...
func (s *LightAroeum) ResetWithGenesisBlock(gb *types.Block) {
	s.blockchain.ResetWithGenesisBlock(gb)
}

func (s *LightAroeum) BlockChain() *light.LightChain      { return s.blockchain }
func (s *LightAroeum) TxPool() *light.TxPool              { return s.txPool }
func (s *LightAroeum) Engine() consensus.Engine           { return s.engine }
func (s *LightAroeum) LesVersion() int                    { return int(ClientProtocolVersions[0]) }
func (s *LightAroeum) Downloader() *downloader.Downloader { return s.handler.downloader }
func (s *LightAroeum) EventMux() *event.TypeMux           { return s.eventMux }
func (s *LightAroeum) Merger() *consensus.Merger          { return s.merger }

// Protocols returns all the currently configured network protocols to start.
func (s *LightAroeum) Protocols() []p2p.Protocol {
...
// light aroeum protocol implementation.
func (s *LightAroeum) Start() error {
