...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightAroeum) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	p.Log().Debug("Light Aroeum peer connected", "name", p.Name())
	...
	if err := p.Handshake(h.backend.blockchain.Genesis().Hash(), forkid, h.forkFilter); err != nil {
		p.Log().Debug("Light Aroeum handshake failed", "err", err)
		return err
	}
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light Aroeum peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("Light Aroeum message handling failed", "err", err)
			p.fcServer.DumpLogs()
			return err
		}
	}
...
func (h *clientHandler) handleMsg(p *serverPeer) error {
	...
	p.Log().Trace("Light Aroeum message arrived", "code", msg.Code, "bytes", msg.Size)
