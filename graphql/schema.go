...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Aroeum address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Aroeum account at a particular block.
    type Account {
    ...
    # Transaction is an Aroeum transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Aroeum account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Aroeum account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Aroeum block by number or by hash. If neither is
