...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of garo is "garo". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Garo", keep that.
	if name == "garo" || name == "garo-testnet" {
		name = "Garo"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "garo" instances.
var isOldGaroResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by garo 1.4 are used if they exist.
	if warn, isOld := isOldGaroResource[path]; isOld {
		oldpath := ""
		if c.name() == "garo" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGaroResourceWarning {
				c.oldGaroResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'garo' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-aroeum-keystore")
		isEphemeral = true
	}
