...
// Package accounts implements high level Aroeum account management.
package accounts
...
// Account represents an Aroeum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Aroeum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain aroeum.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Aroeum Signed Message:\n%d%s", len(data), string(data))
