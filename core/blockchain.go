...
// Package core implements the Aroeum consensus protocol.
package core
...
import (
	...
	"github.com/aroeum-network/go-aroeum/internal/syncx"
...
// available in the database. It initialises the default Aroeum Validator
// and Processor.
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	// If Garo is initialized with an external ancient store, re-initialize the
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		//
		// But if the `TxIndexTail` is not nil, e.g. Garo is initialized with
...
// it's not checked in the Garo side anymore.
...
// Garo will automatically construct the missing indices or delete the extra
// indices.
