// Copyright 2022 The go-aroeum Authors
...
// the process. This is usually the case when Garo is shutting down and some events
...
//
// Opposed to the original Aroeum block synchronization which is trustless (and
...
// not from within (vs. legacy Aroeum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Garo is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Garo requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
