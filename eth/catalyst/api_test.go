...
import (
	...
	beaconConsensus "github.com/aroeum-network/go-aroeum/consensus/beacon"
...
var (
	...
	// testAddr is the Aroeum address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *eth.Aroeum) {
	t.Helper()
...
func setupBlocks(t *testing.T, ethservice *eth.Aroeum, n int, parent *types.Header, callback func(parent *types.Header), withdrawals [][]*types.Withdrawal) []*types.Header {
	api := NewConsensusAPI(ethservice)
...
func setupBodies(t *testing.T) (*node.Node, *eth.Aroeum, []*types.Block) {
	genesis, blocks := generateMergeChain(10, true)
	n, ethservice := startEthService(t, genesis, blocks)
