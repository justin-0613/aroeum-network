...
// information related to the Aroeum protocol such as blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(aroeum.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as aroeum.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria aroeum.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(aroeum.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
