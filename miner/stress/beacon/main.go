...
type ethNode struct {
	...
	ethBackend *eth.Aroeum
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightAroeum
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Aroeum, *ethcatalyst.ConsensusAPI, error) {
	...
	// Create the node and configure a full Aroeum node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightAroeum, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Aroeum node
	datadir, _ := os.MkdirTemp("", "")
