...
// https://github.com/aroeum-network/execution-apis/blob/main/src/engine/specification.md#payloadattributesv1
...
func (payload *Payload) update(block *types.Block, fees *big.Int, elapsed time.Duration) {
	...
	if payload.full == nil || fees.Cmp(payload.fullFees) > 0 {
		...
		feesInAro := new(big.Float).Quo(new(big.Float).SetInt(fees), big.NewFloat(params.Aro))
		log.Info("Updated payload", "id", payload.id, "number", block.NumberU64(), "hash", block.Hash(),
			"txs", len(block.Transactions()), "gas", block.GasUsed(), "fees", feesInAro,
